...
% Sets and functions
% Set notation - most defined in unified_notation.tex
...
% Set notation, probability and error rates - defined in unified_notation.tex
% Note: \error should use \errorrate from unified_notation.tex
...
\begin{example}[Bloom Filter Implementation Insights]
Bloom filters achieve high effective rank through:
\begin{itemize}
    \item \textbf{Subset preservation structure**: Implementation ensures observed sets are supersets of latent sets
    \item \textbf{Hash independence**: Multiple hash functions create near-orthogonal error patterns
    \item \textbf{Bit array sparsity**: Structure prevents many impossible transitions
\end{itemize}

Implementation should maintain these structural properties rather than treating errors as generic noise.
\end{example}

\subsection{Compositional vs Direct Implementation}

Following Part 4's analysis of composite types, implementation strategies differ:

\begin{definition}[Compositional Implementation]
Approximate each component independently, then compose:
\begin{verbatim}
struct CompositionalPair {
    bernoulli<A> first;   // Independent approximation
    bernoulli<B> second;  // Independent approximation
};
\end{verbatim}

\textbf{Advantages**: Modular, reuses existing approximation algorithms
\textbf{Disadvantages**: Higher total error rates, no correlation exploitation
\end{definition}

\begin{definition}[Direct Implementation]
Approximate the composite type as a unit:
\begin{verbatim}
using DirectPair = bernoulli<std::pair<A, B>>;
// Single approximation algorithm for entire pair
\end{verbatim}

\textbf{Advantages**: Lower error rates, can exploit correlation structure
\textbf{Disadvantages**: Requires custom algorithms for each composite type
\end{definition}
