...
\subsection{Confusion Matrix Scales for Functions}

Function approximation exhibits different confusion matrix structures depending on the function space size:

\begin{definition}[Small vs Large Function Spaces]
\textbf{Small Function Spaces:} For functions $X \mapsto Y$ with $|Y|^{|X|}$ small (e.g., $\Bool \mapsto \Bool$ has 4 functions), we can construct and analyze the complete confusion matrix explicitly.

\textbf{Large Function Spaces:} When $|Y|^{|X|}$ is large, the confusion matrix becomes exponentially large. However, \emph{structural patterns} imposed by specific algorithms still determine:
\begin{itemize}
    \item \textbf{Sparsity structure}: Which function-to-function transitions are impossible
    \item \textbf{Rank properties**: Whether structure enhances or reduces inferability
    \item \textbf{Information limits**: What can theoretically be recovered about latent functions
\end{itemize}
\end{definition}

\subsection{Function Confusion Matrix}

When the domain and codomain are finite, we can represent a Bernoulli map using a confusion matrix:
...
\begin{example}[Small Function Space: $\Bool \mapsto \Bool$]
The Boolean function space $\{\mathsf{id}, \mathsf{not}, \mathsf{true}, \mathsf{false}\}$ has only 4 functions, enabling complete confusion matrix analysis in Figure~\ref{fig:bool-func-confusion}.
\begin{figure}[t]
\centering
...
\end{tabular}
\caption{Complete confusion matrix for $\Bool\to\Bool$ under first-order (symmetric) error model.}
\label{fig:bool-func-confusion}
\end{figure}
\textbf{Analysis:}
\begin{itemize}
    \item \textbf{Order}: 1 (single parameter $\epsilon$)
    \item \textbf{Rank}: 4 when $\epsilon \neq 3/4$ (full rank, maximum inferability)
    \item \textbf{Entropy}: Maximum entropy over off-diagonal errors
\end{itemize}
\end{example}

\begin{example}[Large Function Space: Hash Functions]
Consider hash functions $h: \{0,1\}^* \mapsto \{0,1\}^k$ that approximate latent bijections. The confusion matrix has $(2^k)^{2^{|input|}} \times (2^k)^{2^{|input|}}$ entries—intractable to construct explicitly.

\textbf{Structural Properties:}
\begin{itemize}
    \item \textbf{Collision structure}: Functions that agree on most inputs are more likely to be confused
    \item \textbf{Hash quality}: Good hash functions maximize effective rank by minimizing structured collisions
    \item \textbf{Sparsity patterns}: Many function-to-function transitions have probability 0 due to cryptographic properties
\end{itemize}
\end{example}
...
The confusion matrix framework reveals fundamental limits on what can be learned about latent functions, even with unlimited computational resources.

\begin{theorem}[Asymptotic Function Indistinguishability]
Consider two latent functions $f, g: X \to Y$ and their observations through confusion matrix $Q$. If the observed outputs $\obs{f}(x)$ and $\obs{g}(x)$ lie in the same equivalence class under the rank-deficient observation process, then $f$ and $g$ remain asymptotically indistinguishable regardless of the number of observations.
\end{theorem}

\begin{proof}
When $\text{rank}(Q) < |Y|$, the observation process projects the output space $Y$ onto a lower-dimensional subspace. Functions that map to distinct outputs in the null space of $Q$ produce identical observation distributions, making them fundamentally indistinguishable.
\end{proof}

This has profound implications for function approximation and learning:

\begin{itemize}
    \item \textbf{Approximation barriers}: No amount of sampling can distinguish functions whose outputs differ only in the null space of the observation matrix
    \item \textbf{Privacy preservation}: Functions can be designed to be indistinguishable by ensuring their differences lie in rank-deficient subspaces
    \item \textbf{Learning bounds}: Some function classes are inherently unlearnable due to rank constraints, not sample complexity
\end{itemize}

\begin{remark}[Implications for Algorithm Design]
Traditional complexity analysis focuses on computational resources required to solve problems exactly. Rank-based analysis reveals which problems become fundamentally unsolvable when observations are imperfect, regardless of computational resources. This creates a hierarchy of knowable vs unknowable function properties that persists even as computational power increases.
\end{remark}
